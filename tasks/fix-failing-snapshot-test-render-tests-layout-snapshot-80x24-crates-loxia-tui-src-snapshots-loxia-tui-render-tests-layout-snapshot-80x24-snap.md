# Fix failing snapshot test render::tests::layout_snapshot_80x24 (crates/loxia-tui/src/snapshots/loxia_tui__render__tests__layout_snapshot_80x24.snap)

- title: Fix failing snapshot test render::tests::layout_snapshot_80x24 (crates/loxia-tui/src/snapshots/loxia_tui__render__tests__layout_snapshot_80x24.snap)
- description: Fixture: `crates/loxia-tui/src/snapshots/loxia_tui__render__tests__layout_snapshot_80x24.snap`
Test: `render::tests::layout_snapshot_80x24` in `crates/loxia-tui/src/render.rs`
...
Background: on a clean `master`, `cargo test --workspace` fails five insta snapshot tests in loxia-tui. Each one is its own task, and this task covers only the fixture above. The sibling fixtures are layout_snapshot_120x30, layout_snapshot_200x50, now_playing_snapshot_history and header_snapshot_offline_with_downloads. Do not edit their `.snap` files.
...
2. Run `cargo test -p loxia-tui render::tests::layout_snapshot_80x24`. Capture the insta diff from the generated `.snap.new` or from `cargo insta test -p loxia-tui --review`. Do not accept it yet.
...
4. Classify the diff as one of:
   ...
   (b) Rendering regression: the change was not intended. Examples are a truncated or misaligned column, a lost indicator, text that contradicts `docs/`, or a keybinding hardcoded in UI text instead of rendered through `KeyMap::hint_for(ActionId)`. At 80x24, look especially at column-width truncation and whether the hint line and player bar still fit.
...
- (b): fix the rendering in `render.rs` (or the loxia-tui code it calls) so the output matches the stored fixture again. Do not edit the `.snap`. `render.rs` is shared with the 120x30 and 200x50 layout tests, so keep the fix minimal. Say in the PR which of those it also affects.
...
Done when:
- `render::tests::layout_snapshot_80x24` passes.
...
A reviewer checks the work by reading the cited commit next to the snapshot or code diff.
